...
async function sendSmtpMail({
  ...
  const boundary = `gv-quotation-${crypto.randomUUID()}`;
  ...
  try {
    ...
    await sendSmtpCommand(connection, bufferState, `MAIL FROM:<${fromEmail}>`, [250]);
    await sendSmtpCommand(connection, bufferState, `RCPT TO:<${to}>`, [250, 251]);
...
Deno.serve(async (request) => {
  ...
  const smtpConfigured =
    ...
    smtpConfig.password && smtpConfig.fromEmail;
