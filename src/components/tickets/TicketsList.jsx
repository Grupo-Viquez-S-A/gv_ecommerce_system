...
export default function TicketsList({ tickets, search, statusFilter, isLoading, onSearchChange, onStatusChange, onSelect }) {
  return (
    <section>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-lg font-bold text-white">Mis solicitudes</h2>
          <p className="mt-1 text-xs text-gray-400">Consulta tus solicitudes y continúa la conversación con TI.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">{tickets.map((ticket) => <TicketCard key={ticket.id} ticket={ticket} onOpen={onSelect} />)}</div>
      ) : (
